...
const getEnv = (): EnvConfig => {
  ...
  if (missingEnv.length > 0) {
    if (envStatus === 'production') {
      ...
      throw new Error(
        `💥 [CRITICAL ERROR] Fatal: ${missingEnv.length} variable(s) must be defined in production.`
      );
    } else {
      console.warn(
        `⚠️  [SECURITY WARNING] Missing environment variables: ${missingEnv.join(', ')}. Using fallbacks.`
      );
    }
  }
  ...
  return {
    ...
    FIREBASE_CLIENT_EMAIL:
      process.env.FIREBASE_CLIENT_EMAIL || 'firebase-adminsdk@agriflux.iam.gserviceaccount.com',
