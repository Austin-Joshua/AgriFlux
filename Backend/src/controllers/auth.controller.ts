...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    if (!password || !name || (!email && !phone)) {
      return res
        .status(400)
        .json({ message: 'Please provide email or phone, plus password and name.' });
    }

    if (!isDBConnected) {
      return res
        .status(503)
        .json({ message: 'Database connecting, please try again in a moment.' });
    }
    ...
    const token = jwt.sign({ id: newUser._id, phone: newUser.phone }, JWT_SECRET, {
      expiresIn: '7d',
    });
...
export const socialLogin = async (req: Request, res: Response) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ success: false, message: 'Email is required from Google account.' });
    }
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[PASSWORD RESET] Link: http://localhost:3000/reset-password?token=${resetToken}&email=${encodeURIComponent(email)}`
    );
