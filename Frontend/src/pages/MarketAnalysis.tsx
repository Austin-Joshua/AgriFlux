...
const MarketAnalysis: React.FC = () => {
  ...
  const filtered = marketData.filter((c) => c.name.toLowerCase().includes(search.toLowerCase()));

  const chartData =
    timeframe === '1D'
      ? selectedCrop.trends
      : timeframe === '1W'
        ? generateData(selectedCrop.price, 6, weekLabels)
        : timeframe === '1M'
          ? generateData(selectedCrop.price, 4, monthLabels)
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-lg flex items-center gap-1 ${selectedCrop.change.startsWith('+') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                    >
                      {selectedCrop.change.startsWith('+') ? (
                        <TrendingUp size={12} />
                      ) : (
              ...
              <button
                onClick={() => toggleWatchlist(selectedCrop.name)}
                className={`p-2 rounded-xl border transition-all ${watchlist.includes(selectedCrop.name) ? 'bg-amber-100 border-amber-200 text-amber-600' : 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-400'}`}
              >
                <Star
                  size={18}
                  fill={watchlist.includes(selectedCrop.name) ? 'currentColor' : 'none'}
                />
              </button>
            </div>
            ...
            {/* MSP Alert */}
            <div
              className={`p-3 rounded-xl mb-6 border ${selectedCrop.price >= selectedCrop.msp ? 'bg-primary-50 dark:bg-primary-900/10 border-primary-100 dark:border-primary-800/50 text-primary-700 dark:text-primary-400' : 'bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-800/50 text-red-700 dark:text-red-400'}`}
            >
              <p className="text-xs font-bold flex items-center gap-2">
                <AlertCircle size={14} />
                {selectedCrop.price >= selectedCrop.msp
                  ? t('market.aboveMsp', {
                      diff: (selectedCrop.price - selectedCrop.msp).toLocaleString(),
                    })
                  : t('market.belowMsp', {
                      diff: (selectedCrop.msp - selectedCrop.price).toLocaleString(),
                      msp: selectedCrop.msp.toLocaleString(),
                    })}
              </p>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                {timeframe === '1D' || timeframe === '1M' ? (
                  <ComposedChart data={chartData}>
                    ...
                    />
                    <Bar dataKey={['low', 'high'] as unknown as string} shape={<Candlestick />} />
                  </ComposedChart>
                ) : (
              ...
              {[
                {
                  ...
                  value: `₹${(selectedCrop.price + 120).toLocaleString()}`,
                },
                {
                  label: t('market.dayLow'),
                  value: `₹${(selectedCrop.price - 85).toLocaleString()}`,
                },
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {marketData
                .filter((d) => watchlist.includes(d.name))
                .map((d) => (
                  <div
                    key={d.name}
                    onClick={() => setSelectedCrop(d)}
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] font-bold ${d.change.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}
                    >
                      {d.change}
                    </span>
                  </div>
              ...
              {watchlist.length === 0 && (
                <p className="text-xs text-gray-400 py-4 italic">{t('market.starToWatch')}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="card">
            ...
            <h3 className="section-header mb-4 flex items-center gap-2">
              <ArrowUpDown size={14} /> {t('market.topMovers')}
            </h3>
          ...
          <div className="card bg-gray-900 text-white border-none shadow-glow-green overflow-hidden relative">
            <div className="relative z-10">
              <h3 className="font-black text-lg mb-4">
                {t('market.gainers')} & {t('market.losers')}
              </h3>
              <div className="space-y-4">
                <div className="p-3 bg-white/5 rounded-xl border border-white/10">
                  <p className="text-[10px] font-bold text-green-400 uppercase tracking-widest mb-2 flex items-center gap-1">
                    <TrendingUp size={10} /> {t('market.gainers')}
                  </p>
                ...
                <div className="p-3 bg-white/5 rounded-xl border border-white/10">
                  <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest mb-2 flex items-center gap-1">
                    <TrendingDown size={10} /> {t('market.losers')}
                  </p>
                  <div className="flex justify-between items-end">
