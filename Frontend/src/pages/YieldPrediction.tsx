...
const predictYield = (inputs: Record<string, number | string>) => {
  ...
  const n = Number(inputs.nitrogen),
    p = Number(inputs.phosphorus),
    k = Number(inputs.potassium);
  const rain = Number(inputs.rainfall),
    temp = Number(inputs.temperature);
  ...
  const predicted = Math.round(
    base * nutrientScore * weatherScore * tempScore * fertScore * humidityScore
  );
  const historical = Number(inputs.historicalYield) || base;
  const improvement = (((predicted - historical) / historical) * 100).toFixed(1);
  const risk =
    predicted < historical * 0.85 ? 'High' : predicted < historical * 0.95 ? 'Medium' : 'Low';
    ...
    if (label === 'Predicted Yield') {
      ...
      content = (
        <div className="space-y-4">
          <p>
            Current Predicted Yield: <strong>{val} kg</strong>
          </p>
          <p>
            This prediction uses a multi-factor crop yield model incorporating nutrient availability
            (NPK), rainfall, temperature, humidity, and fertilizer usage. The model is calibrated
            against regional baseline yields for each crop type.
          </p>
        </div>
      );
      type = 'success';
      ...
      content = (
        <div className="space-y-4">
          <p>
            Projected Improvement: <strong>{val}%</strong>
          </p>
          <p>
            Improvement is measured against your baseline historical yield. A positive percentage
            indicates that current management practices and favorable weather are likely to result
            in a better harvest.
          </p>
        </div>
      );
      ...
      content = (
        <div className="space-y-4">
          <p>
            Yield Risk: <strong>{val}</strong>
          </p>
          <p>
            Risk assessment evaluates the probability of yield loss due to biotic or abiotic stress
            factors.
          </p>
        </div>
      );
      ...
      content = (
        <div className="space-y-4">
          <p>
            Model Confidence: <strong>99%</strong>
          </p>
          <p>
            Calculated based on the completeness of input data and the prediction model's historical
            accuracy for this crop in this region.
          </p>
        </div>
      );
      type = 'info';
    ...
    <div className="space-y-6">
        ...
        {[
          {
            ...
            value: result ? `${result.predicted} kg` : '---',
          ...
          },
          {
            ...
            value: form.historicalYield ? `${form.historicalYield} kg` : '---',
            color: 'text-blue-600',
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="card glass-gold flex flex-col border-gold-200 dark:border-gold-900/40">
          ...
          {result && (
            <div
              className={`mt-4 p-5 rounded-2xl border-b-4 shadow-xl transition-all animate-scale-in ${
                result.risk === 'Low'
            ...
            >
              ...
              <div className="flex items-center gap-3 mt-3">
                <span
                  className={`badge py-1 px-3 ${result.risk === 'Low' ? 'badge-green' : result.risk === 'Medium' ? 'badge-gold' : 'badge-red'}`}
                >
                  {result.risk} Risk
                </span>
                <span
                  className={`flex items-center gap-1 text-sm font-bold ${Number(result.improvement) >= 0 ? 'text-primary-600 dark:text-primary-400' : 'text-red-500'}`}
                >
                  {Number(result.improvement) >= 0 ? <TrendingUp size={14} /> : null}
                  {Number(result.improvement) >= 0 ? '+' : ''}
                  {result.improvement}%
                </span>
              </div>
            </div>
