...
const StatCard: React.FC<{
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none shadow-glow-green"
        style={{ background: `linear-gradient(135deg, ${accentColor}10 0%, transparent 60%)` }}
      />
      <div className="flex items-start justify-between mb-2">
        <div
          className={`w-12 h-12 ${bgColor} rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300`}
        >
          {icon}
        </div>
        <div className="flex flex-col items-end gap-1">
          <span
            className={`flex items-center gap-1 text-sm font-semibold ${positive ? 'text-primary-600 dark:text-primary-400' : 'text-red-500'}`}
          >
    ...
    <div className="space-y-4">
      ...
      <SmartFarmReport
        ...
        reportData={{
          ...
          riskLevel:
            data.confidenceScore >= 93 ? 'Low' : data.confidenceScore >= 88 ? 'Medium' : 'High',
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="page-header">
            {t(`dashboard.${greeting.toLowerCase().replace(' ', '')}`)},{' '}
      ...
      <div
        className="grid grid-cols-2 lg:grid-cols-3 gap-4 animate-slide-up-fade"
      ...
      {/* Charts Row */}
      <div
        className="grid grid-cols-1 lg:grid-cols-3 gap-4 animate-slide-up-fade"
      ...
      >
        ...
        <div className="lg:col-span-2 card">
          ...
          <div
            ...
            onClick={() =>
              toast.info(
                '📊 AI Insight: Yield expected to peak in May due to optimized irrigation.'
              )
            }
          >
            <div
              role="img"
              aria-label="Crop Yield Trend Chart comparing actual yield and AI forecast over the last 6 months"
            >
        ...
        {/* 7-Day Weather */}
        <div
          className="card group/weather cursor-pointer hover:border-blue-500/30 transition-all"
          onClick={() => toast.success('🌤️ Weather Analysis: Stable conditions for harvest.')}
        >
          <div className="flex items-center justify-between mb-2">
            <h3 className="section-header">7-Day Weather</h3>
            <Droplets
              size={18}
              className="text-blue-400 group-hover/weather:scale-125 transition-transform"
            />
          </div>
          <div className="space-y-2">
            {weatherData.map((d) => (
              <div
                key={d.day}
                className="flex items-center gap-3 py-1 border-b border-gray-50 dark:border-gray-700/30 last:border-0 group/row"
              >
                <span className="text-sm text-gray-500 dark:text-gray-400 w-8 group-hover/row:text-primary-600 transition-colors">
                  {d.day}
                </span>
                ...
                <span className="text-sm font-bold text-amber-500 dark:text-amber-400 w-8 text-right">
                  {d.temp}°
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {/* Farm Map */}
        <div
          className="card !p-0 overflow-hidden animate-slide-up-fade"
        ...
        >
          ...
          <div className="relative w-full group/map" style={{ height: '180px' }}>
            ...
            <div className="absolute bottom-3 left-3 right-3 flex justify-between items-end opacity-0 group-hover/map:opacity-100 transition-opacity duration-300 pointer-events-none">
              <div className="glass px-2 py-1 rounded-lg text-[10px] font-bold text-white uppercase tracking-tighter">
                Lat: 12.95° N · Lon: 77.63° E
              </div>
              <div className="flex gap-1 pointer-events-auto">
                <button
                  className="p-1.5 bg-black/60 hover:bg-primary-600 rounded-lg text-white transition-all backdrop-blur-sm"
                  onClick={() => toast.info('🗺️ Map Layer: Terrain Enabled')}
                >
                  <Map size={12} />
                </button>
      ...
      <div className="card animate-slide-up-fade" style={{ animationDelay: '380ms' }}>
        <div className="flex items-center justify-between mb-4 border-b border-gray-100 dark:border-gray-800 pb-3">
          <div>
            ...
            <p className="text-gray-400 dark:text-gray-500 text-xs mt-0.5">
              Track requested sessions and their confirmation status
            </p>
          </div>
          <button
        ...
        {loadingBookings ? (
          ...
          <div className="text-center py-6">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Connect with our certified network of soil & crop experts to optimize your farm yield.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {bookings.slice(0, 3).map((b) => (
              <div
                key={b._id}
                className="p-4 rounded-2xl bg-gray-50/50 dark:bg-gray-800/20 border border-gray-100 dark:border-gray-800 flex flex-col justify-between gap-3 hover:shadow-sm transition-all"
              >
                <div>
                  ...
                  <h4 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight mt-3">
                    {b.expertName}
                  </h4>
                ...
                <div className="flex items-center justify-between border-t border-gray-100 dark:border-gray-800/50 pt-2 text-[11px] text-gray-500 font-medium mt-1">
                  ...
                  {b.status === 'confirmed' && (
                    <span className="flex items-center gap-1 text-primary-500 font-bold">
                      <Video size={11} /> Link Sent
                    </span>
                  )}
                </div>
              </div>
      ...
      <div
        className="flex flex-wrap items-center justify-center gap-4 pt-2 pb-1 animate-slide-up-fade"
      ...
      >
        {[
          ...
          <div
            key={label}
            className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 rounded-full border border-gray-100 dark:border-gray-800"
          >
