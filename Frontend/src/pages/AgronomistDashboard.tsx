...
const AgronomistDashboard: React.FC = () => {
    ...
    const sendAdvisory = () => {
        ...
        setSentAdvisories(prev => [{ to: 'All Monitored Farmers', msg: advisoryText, time: new Date().toLocaleTimeString() }, ...prev]);
        setAdvisoryText('');
    };
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div>
                    <h1 className="page-header">🔬 Agronomist Dashboard</h1>
                    <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Welcome, <strong>{user?.name}</strong> · {user?.expertise}</p>
                </div>
                <div className="flex gap-2">
                    <span className="px-3 py-1.5 rounded-xl bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-xs font-bold border border-blue-200 dark:border-blue-700/50">🔬 Agronomist Mode</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    { label: 'Farms Monitored', value: '42', icon: '🌾', color: 'text-primary-600' },
                    { label: 'Field Visits (Month)', value: '13', icon: '🚗', color: 'text-blue-600' },
                    { label: 'Active Cases', value: '24', icon: '⚠️', color: 'text-amber-600' },
                    { label: 'Advisories Sent', value: '156', icon: '📨', color: 'text-purple-600' },
                ].map(s => (
            ...
            <div className="flex gap-2 bg-gray-100 dark:bg-gray-700 rounded-xl p-1 w-fit flex-wrap">
                {(['overview', 'soilReports', 'fieldVisits', 'advisory'] as const).map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)}
                        ...
                        {tab === 'overview' ? '📊 Overview' : tab === 'soilReports' ? '🧪 Soil Reports' : tab === 'fieldVisits' ? '🗺️ Field Visits' : '📡 Advisory'}
                    </button>
                ))}
            </div>
            ...
            {activeTab === 'overview' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div className="lg:col-span-2 card">
                        <h3 className="section-header mb-4">Field Visits — Last 6 Weeks</h3>
                        ...
                        <div className="mt-4">
                            <h4 className="text-sm font-bold text-gray-700 dark:text-gray-200 mb-3 flex items-center gap-2"><AlertTriangle size={14} className="text-red-500" /> Active Disease Alerts</h4>
                                    ...
                                    <div key={a.disease} className="flex items-center justify-between p-2.5 rounded-xl bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-800/30">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-gray-500">{a.farms} farms · {a.region}</p>
                                            </div>
                                        </div>
                    ...
                    <div className="card">
                        <h3 className="section-header mb-4">Case Status</h3>
            ...
            {activeTab === 'soilReports' && (
                <div className="card">
                    <h3 className="section-header mb-4">🧪 Soil Test Reports from Monitored Farms</h3>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead><tr className="border-b border-gray-100 dark:border-gray-700">{['Farm', 'Farmer', 'pH', 'Nitrogen', 'Phosphorus', 'Potassium', 'Salinity', 'Status', 'Date'].map(h => <th key={h} className="text-left py-2 px-3 text-xs font-bold text-gray-400 uppercase tracking-wide">{h}</th>)}</tr></thead>
                            <tbody>
                                {soilReports.map(r => (
            ...
            {activeTab === 'fieldVisits' && (
                <div className="space-y-3">
                    <h3 className="section-header">📅 Upcoming Field Visits</h3>
            ...
            {activeTab === 'advisory' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div className="card">
                        <h3 className="section-header mb-4 flex items-center gap-2"><Send size={15} /> Send Field Advisory</h3>
                        <div className="space-y-3">
                            <div>
                                <label className="label">Target Audience</label>
                                <select className="input-field">
                                    <option>All Monitored Farmers (42)</option>
                                    <option>Kansas Region Farmers</option>
                                    <option>Wheat Crop Farmers</option>
                                    <option>High-Risk Farms Only</option>
                                </select>
                            </div>
                            <div>
                                <label className="label">Advisory Message</label>
                                <textarea className="input-field min-h-[120px] resize-none" placeholder="e.g. Due to forecasted heavy rains next week, delay fertilisation by 5 days. Apply 2% urea foliar spray on wheat crops showing nitrogen deficiency..." value={advisoryText} onChange={e => setAdvisoryText(e.target.value)} />
                            </div>
                            <button onClick={sendAdvisory} className="btn-primary w-full flex items-center justify-center gap-2 py-3">
                                <Send size={14} /> Send Advisory to Farmers
                            </button>
                        </div>
                    </div>
                    <div className="card">
                        <h3 className="section-header mb-4">📨 Sent Advisories</h3>
                        {sentAdvisories.length === 0 && <p className="text-sm text-gray-400 text-center py-8">No advisories sent yet. Compose one on the left and send it.</p>}
                        <div className="space-y-3">
                            {sentAdvisories.map((a, i) => (
                                <div key={i} className="p-3 rounded-xl bg-primary-50 dark:bg-primary-900/20 border border-primary-100 dark:border-primary-800/50">
                                    <div className="flex items-center justify-between mb-1">
                                        <p className="text-xs font-bold text-primary-600 dark:text-primary-400">To: {a.to}</p>
