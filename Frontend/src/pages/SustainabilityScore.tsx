    ...
    if (label === 'Fertilizer Efficiency') {
      content = (
        <div className="space-y-4">
          <p>
            Current Efficiency: <strong>{fertEff}%</strong>
          </p>
          <p>
            Fertilizer efficiency measures the amount of applied nutrients that are actually taken
            up by the crop versus those lost to leaching or volatilization.
          </p>
          <div className="bg-primary-50 dark:bg-primary-900/20 p-4 rounded-xl border border-primary-100 dark:border-primary-800">
            ...
            <p className="text-sm">
              Your split-application strategy has significantly reduced Nitrogen leaching. Continued
              use of organic amendments will further stabilize these levels.
            </p>
          </div>
        </div>
      );
      ...
      content = (
        <div className="space-y-4">
          <p>
            Current Conservation Score: <strong>{waterCons}%</strong>
          </p>
          <p>
            This score reflects how effectively you managed your water resources relative to the
            theoretical crop water requirement.
          </p>
        </div>
      );
      type = 'info';
      ...
      content = (
        <div className="space-y-4">
          <p>
            Diversity Index: <strong>{cropDiv}%</strong>
          </p>
          <p>
            Crop diversity is critical for pest management and long-term soil vitality. More diverse
            systems are naturally more resilient to climate shocks.
          </p>
        </div>
      );
      type = 'info';
      ...
      content = (
        <div className="space-y-4">
          <p>
            Soil Health Index: <strong>{soilHealth}/100</strong>
          </p>
          <p>
            A comprehensive rating based on organic carbon, microbial activity, and structure
            stability.
          </p>
        </div>
      );
  ...
  const metrics = [
    {
      ...
      icon: Leaf,
      color: 'primary',
      tip: 'Reduce chemical fertilizer by 15% with compost blending',
    },
    {
      ...
      icon: Droplets,
      color: 'blue',
      tip: 'Drip irrigation can save an additional 30% water',
    },
    {
      ...
      icon: RefreshCcw,
      color: 'earth',
      tip: 'Add 1 more crop variety to boost biodiversity score',
    },
    {
      ...
      icon: TrendingUp,
      color: 'primary',
      tip: 'Excellent! Maintain organic matter levels for sustained health',
    },
  ];

  const scoreColor = overall >= 80 ? '#22c55e' : overall >= 60 ? '#f59e0b' : '#ef4444';
  const scoreLabel =
    overall >= 80 ? 'Excellent 🌟' : overall >= 60 ? 'Good 👍' : 'Needs Improvement ⚠️';
        ...
        <div className="card flex flex-col items-center py-8">
          ...
          <div className="mt-6 w-full grid grid-cols-2 gap-2">
            {[
              ...
              <div
                key={s.label}
                className={`p-2.5 ${s.color === 'blue' ? 'bg-blue-50 dark:bg-blue-900/20' : s.color === 'earth' ? 'bg-earth-50 dark:bg-earth-900/20' : 'bg-primary-50 dark:bg-primary-900/20'} rounded-xl text-center`}
              >
                <p
                  className={`font-bold text-sm ${s.color === 'blue' ? 'text-blue-600' : s.color === 'earth' ? 'text-earth-600' : 'text-primary-600'}`}
                >
                  {s.value}
                </p>
          ...
          {metrics.map((m) => (
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                <div
                  className={`w-9 h-9 ${m.color === 'blue' ? 'bg-blue-100 dark:bg-blue-900/30' : m.color === 'earth' ? 'bg-earth-100 dark:bg-earth-900/30' : 'bg-primary-100 dark:bg-primary-900/30'} rounded-xl flex items-center justify-center group-hover:rotate-12 transition-transform`}
                >
                  <m.icon
                    size={16}
                    className={
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[7px] font-black text-primary-500 opacity-0 group-hover:opacity-100 transition-opacity">
                      Full Report
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-1.5 mr-3 overflow-hidden">
                      <div
                        className={`h-1.5 rounded-full ${m.color === 'blue' ? 'bg-blue-500' : m.color === 'earth' ? 'bg-earth-500' : 'bg-primary-500'}`}
                        style={{ width: `${m.value}%` }}
                      />
                    </div>
