...
const AdminDashboard: React.FC = () => {
  ...
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'activity' | 'system'>(
    'overview'
  );
  ...
  const activityLog = [
    {
      id: 1,
      user: 'user@agriflux.ai',
      action: 'Ran Soil Analysis',
      time: '10 min ago',
      role: 'Farmer',
      status: 'success',
    },
    {
      id: 2,
      user: 'agronomist@agriflux.ai',
      action: 'Sent bulk advisory',
      time: '23 min ago',
      role: 'Agronomist',
      status: 'success',
    },
  ...
  const filteredUsers = userList.filter(
    (u) => userFilter === 'All' || u.role === userFilter || u.status === userFilter
  );
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            {t('admin.loggedInAs')} <strong>{user?.name}</strong> · {t('admin.fullAccess')}
          </p>
        </div>
        <div className="flex items-center justify-center md:justify-start gap-2">
          <span className="px-3 py-1.5 rounded-xl bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-xs font-bold border border-red-200">
            🛡️ {t('admin.mode')}
          </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            value: '43',
            sub: t('admin.rightNow'),
            icon: '🟢',
            color: 'text-green-600',
          },
          {
            label: t('admin.securityAlerts'),
            value: '2',
            sub: t('admin.needsReview'),
            icon: '🚨',
            color: 'text-red-600',
          },
          {
            label: t('admin.aiEngine'),
            value: 'Online',
            sub: t('admin.uptime'),
            icon: '🤖',
            color: 'text-blue-600',
            ...
            <div className="mt-3 space-y-1.5">
              {roleData.map((r) => (
                <div key={r.name} className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-gray-700 dark:text-gray-200">{r.value}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                {filteredUsers.map((u) => (
                  ...
                  >
                    ...
                    <td className="py-2.5 px-3">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${u.role === 'Admin' ? 'bg-red-100 text-red-700' : u.role === 'Agronomist' ? 'bg-blue-100 text-blue-700' : 'bg-primary-100 text-primary-700'}`}
                      >
                        {u.role}
                      </span>
                    </td>
                    <td className="py-2.5 px-3">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${u.status === 'Active' ? 'bg-primary-100 text-primary-700' : u.status === 'Inactive' ? 'bg-gray-100 text-gray-500' : 'bg-red-100 text-red-700'}`}
                      >
                        {u.status}
                      </span>
                    </td>
                    <td className="py-2.5 px-3 text-gray-400 text-xs">{u.joined}</td>
                    <td className="py-2.5 px-3 font-semibold text-gray-700 dark:text-gray-200">
                      {u.logins}
                    </td>
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-1">
                        <button className="p-1 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-500 transition-colors">
                          <Eye size={13} />
                        </button>
                        <button className="p-1 rounded hover:bg-amber-50 dark:hover:bg-amber-900/20 text-amber-500 transition-colors">
                          <Settings size={13} />
                        </button>
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            {activityLog.map((log) => (
              <div
                key={log.id}
                className="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-700/30 transition-colors"
              >
                ...
                <div className="text-right flex-shrink-0">
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${log.role === 'Admin' ? 'bg-red-100 text-red-700' : log.role === 'Agronomist' ? 'bg-blue-100 text-blue-700' : 'bg-primary-100 text-primary-700'}`}
                  >
                    {log.role}
                  </span>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {[
                ...
                <div
                  key={s.label}
                  className="flex items-center justify-between p-2.5 rounded-xl border border-gray-100 dark:border-gray-700"
                >
                  <span className="text-sm text-gray-600 dark:text-gray-300">{s.label}</span>
                  <div
                    className={`w-10 h-5 rounded-full transition-colors relative cursor-pointer ${s.enabled ? 'bg-primary-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${s.enabled ? 'right-0.5' : 'left-0.5'}`}
                    />
                  </div>
                </div>
              ))}
