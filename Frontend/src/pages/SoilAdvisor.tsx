...
const SoilAdvisor: React.FC = () => {
  ...
  const [input, setInput] = useState<SoilInput>({
    salinity: '',
    ph: '',
    moisture: '',
    waterLevel: '',
    ...
    const all: CropRec[] = [
      {
        ...
        suitability: '',
        guidance:
          'Paddy thrives in high moisture, neutral to slightly acidic soils. Use SRI method to maximise yield with reduced water.',
        soilPrep: 'Puddle soil 2 weeks before transplant. Apply 25 kg/ha zinc sulphate.',
        waterReq: '1,200–2,000 mm/season',
        yieldPotential: '5–8 MT/ha',
        tenYearTrend:
          'Stable demand; MSP raised 8% avg over last 10 years. Export demand rising for Basmati.',
      ...
      },
      {
        ...
        suitability: '',
        guidance:
          'Well-drained loamy soil, low-medium salinity. Sow October–November for best yield. Use DBW 222 or HD 3226 varieties.',
        soilPrep: 'Deep ploughing + 2 tons FYM. Apply NPK 120:60:40 kg/ha.',
        waterReq: '450–650 mm (4–6 irrigations)',
        yieldPotential: '4–6 MT/ha',
        tenYearTrend:
          'Consistent MSP growth, 6.5% avg CAGR. Climate stress reducing N-India yields; technology gap = opportunity.',
      ...
      },
      {
        ...
        suitability: '',
        guidance:
          'Tolerates moderate salinity and alkalinity. Deep black cotton soil preferred. Use BT varieties for pest management.',
        soilPrep: 'Deep summer ploughing. Apply 10 tons FYM + neem cake.',
        waterReq: '700–1,200 mm (drip preferred)',
        yieldPotential: '1.8–2.5 MT/ha seed cotton',
        tenYearTrend:
          'Export earnings volatile; domestic textile demand strong. GMO technology providing 15% yield advantage.',
      ...
      },
      {
        ...
        suitability: '',
        guidance:
          'Well-drained soil, tolerant of moderate moisture. Suitable for a wide pH range. Excellent feed crop with rising demand.',
        soilPrep: 'Add 2–3 tonnes FYM pre-sowing. NPK 150:75:75 kg/ha.',
        waterReq: '600–900 mm',
        yieldPotential: '6–10 MT/ha (hybrid)',
        tenYearTrend:
          'Ethanol blending policy driving 30% demand surge. Contract farming opportunities with poultry and ethanol plants growing rapidly.',
      ...
      },
      {
        ...
        suitability: '',
        guidance:
          'Drought-tolerant, suits well-drained soil. Sensitive to waterlogging. Good rotation crop for soil health.',
        soilPrep: '1 tonne FYM + 40 kg P2O5/ha pre-sowing. Boron foliar spray at bud stage.',
        waterReq: '500–750 mm (5–6 irrigations)',
        yieldPotential: '1.5–2.5 MT/ha',
        tenYearTrend:
          'Edible oil import dependency creating strong domestic price support. 10-year avg price up 12% CAGR.',
      ...
      },
      {
        ...
        suitability: '',
        guidance:
          'Legume — fixes nitrogen and improves soil. Moderate moisture requirement. Short germination period ideal for quick rotation.',
        soilPrep: 'Rhizobium seed treatment. Add 20 kg P and K each/ha.',
        waterReq: '600–700 mm',
        yieldPotential: '1.5–3 MT/ha',
        tenYearTrend:
          'Protein meal exports booming. MSP revised up 10% in 2023. Strong demand from poultry feed industry.',
        color: 'from-brown-500 to-earth-400',
      },
      {
        name: 'Barley',
        emoji: '🌾',
        ...
        suitability: '',
        guidance:
          'High salinity tolerance (up to 8 dS/m). Cool season crop. Suitable for alkaline soils (pH up to 8.5). Ideal for saline-alkali reclamation.',
        soilPrep: 'Gypsum application (2.5 t/ha) for alkali soils. Balance K before sowing.',
        waterReq: '350–500 mm',
        yieldPotential: '3–5 MT/ha',
        tenYearTrend:
          'Specialty barley for malt and beer industry commands premium of 40–60% above feed grade. Niche market growing.',
      ...
      },
      {
        ...
        suitability: '',
        guidance:
          'High water requirement but tolerates waterlogging for short periods. Plant in February or October. Ratoon crop helps cut costs.',
        soilPrep: 'Deep furrow planting, 5 t FYM. Trash mulching conserves moisture.',
        waterReq: '1,500–2,500 mm',
        yieldPotential: '70–100 MT/ha cane',
        tenYearTrend:
          'FRP revised annually. Ethanol policy guarantees offtake. Co-gen power adds revenue stream. Long-term price stable.',
    ...
    return all
      .map((crop) => {
        ...
        if (crop.name === 'Rice') {
          ...
          if (ph >= 6.0 && ph <= 7.5) s += 15;
          else s -= 10;
          if (sal <= 2.5) s += 10;
          else s -= 15;
          if (moist >= 50 && moist <= 75) s += 10;
          else s -= 10;
  ...
  const analyze = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const warns: string[] = [];
    if (parseFloat(input.salinity) > 6)
      warns.push(
        '⚠️ High salinity detected. Consider leaching with fresh water and gypsum treatment before planting.'
      );
    if (parseFloat(input.ph) > 8.5)
      warns.push(
        '⚠️ Highly alkaline soil. Apply gypsum (2–4 t/ha) and sulphur to reduce pH before sowing.'
      );
    if (parseFloat(input.ph) < 5.0)
      warns.push(
        '⚠️ Highly acidic soil (pH < 5.0). Lime application recommended at 1–2 tonnes/ha to improve nutrient availability.'
      );
    if (parseFloat(input.moisture) < 30)
      warns.push(
        '⚠️ Low soil moisture. Schedule irrigation or mulching before planting to avoid germination failure.'
      );
    if (parseFloat(input.waterLevel) < 0.5)
      warns.push(
        '⚠️ Very shallow water table — waterlogging risk. Raised-bed farming recommended.'
      );
    setAlerts(warns);
    ...
    setReportStats({
      ...
      nitrogen: Math.min(
        100,
        Math.max(15, Math.round((parseFloat(input.germination) || 85) * 0.55))
      ),
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="card">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-gray-900 dark:text-white">
                      {t('soilAdvisor.soilReport')}
                    </h2>
                    <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">
                      {t('soilAdvisor.lastUpdated')}: 2 {t('common.daysAgo')}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                  {[
                        ...
                        reportStats.ph < 5.0
                          ...
                          : reportStats.ph < 6.0
                            ...
                            : reportStats.ph > 8.0
                              ...
                              : 'Optimal',
                      color:
                        reportStats.ph < 5.0 || reportStats.ph > 8.0
                          ? 'text-red-500'
                          : 'text-green-500',
                    },
                    {
                      ...
                      status:
                        reportStats.nitrogen < 30
                          ? 'Deficient'
                          : reportStats.nitrogen > 75
                            ? 'Abundant'
                            : 'Optimal',
                    ...
                    },
                    {
                      ...
                      status:
                        reportStats.potassium < 35
                          ? 'Deficient'
                          : reportStats.potassium > 80
                            ? 'High'
                            : 'Optimal',
                    ...
                    >
                      ...
                      <p className="text-2xl font-black text-gray-900 dark:text-white group-hover/stat:text-primary-600 transition-colors">
                        {stat.value}
                      </p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-2xl bg-primary-50 dark:bg-primary-900/10 border border-primary-100 dark:border-primary-800/50 flex gap-4">
                    ...
                    <div>
                      <p className="font-bold text-gray-900 dark:text-white text-sm">
                        {t('soilAdvisor.irrigationStrategy')}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">
                        Based on current moisture levels, increase watering cycles by 15% for the
                        next 3 days to support peak growth phase.
                      </p>
                    </div>
                  </div>
                  <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/50 flex gap-4">
                    ...
                    <div>
                      <p className="font-bold text-gray-900 dark:text-white text-sm">
                        {t('soilAdvisor.fertilizerPlan')}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">
                        Apply 2.5kg/acre of Phosphorus-rich organic compost to address soil
                        deficiency detected in northern plot B-12.
                      </p>
                    </div>
                  </div>
                </div>
                                ...
                                <div>
                                  ...
                                  <ul className="space-y-3 text-sm">
                                    <li className="flex gap-2">
                                      ...
                                      />{' '}
                                      <strong>Soil Preparation:</strong> {rec.soilPrep}
                                    </li>
                                    <li className="flex gap-2">
                                      <CheckCircle
                                        size={16}
                                        className="text-green-500 shrink-0 mt-0.5"
                                      />{' '}
                                      <strong>Water Requirement:</strong> {rec.waterReq}
                                    </li>
                                    <li className="flex gap-2">
                                      <CheckCircle
                                        size={16}
                                        className="text-green-500 shrink-0 mt-0.5"
                                      />{' '}
                                      <strong>Yield Potential:</strong> {rec.yieldPotential}
                                    </li>
                                  </ul>
                                </div>
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="font-black text-gray-900 dark:text-white group-hover:text-primary-600 transition-colors">
                                #{i + 1} {rec.name}
                              </h4>
                          ...
                          <div className="flex flex-col items-end gap-2">
                            <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-lg bg-primary-100 dark:bg-primary-900/30 text-primary-600">
                              {rec.suitability}
                            </span>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 leading-relaxed">
                          {rec.guidance.substring(0, 90)}...
                        </p>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                          <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase">
                              {t('soilAdvisor.waterReq')}
                            </p>
                            <p className="text-xs font-bold">{rec.waterReq.split(' ')[0]}</p>
                          </div>
                          <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase">
                              {t('soilAdvisor.yieldPotential')}
                            </p>
                            <p className="text-xs font-bold">{rec.yieldPotential.split(' ')[0]}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/50">
                      ...
                      <div className="space-y-1">
                        {scanResult.issues.map((issue, idx) => (
                          <p
                            key={idx}
                            className="text-[11px] font-bold text-amber-700 dark:text-amber-400 flex items-center gap-1.5"
                          >
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {[
                {
                  task: 'Soil Testing Plot C-4',
                  time: 'Tomorrow, 08:00 AM',
                  priority: 'High',
                  color: 'text-red-500',
                },
                {
                  task: 'Foliar Nutrient Spray',
                  time: 'Friday, 06:30 AM',
                  priority: 'Normal',
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${item.priority === 'High' ? 'bg-red-500' : 'bg-primary-500'}`}
                    />
                    <div>
        ...
        <div className="space-y-6">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-black uppercase tracking-wider">
                  {t('soilAdvisor.expertHelp')}
                </h3>
              </div>
              <p className="text-xs text-gray-400 mb-6 leading-relaxed">
                Get a professional agronomy report for your farm by connecting with our certified
                experts.
              </p>
              <button
          ...
          <div className="card">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed italic border-l-2 border-amber-300 pl-3">
              "Current soil moisture level is ideal for Wheat plantation. This could lead to a 10%
              higher yield if sowing is completed this week."
            </p>
          </div>
        </div>
      </div>
