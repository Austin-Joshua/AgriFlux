        ...
        <div className="flex-1 card">
          ...
          {activeSection === 'profile' && (
            <div className="space-y-5">
              <div className="flex items-center gap-4 pb-5 border-b border-gray-100 dark:border-gray-700">
                ...
                <div>
                  <h3 className="font-black text-xl text-gray-900 dark:text-white font-display tracking-tight">
                    {user?.name}
                  </h3>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400 px-2 py-0.5 rounded text-[10px] font-bold">
                      VERIFIED FARMER
                    </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    value: 'dark',
                    icon: Moon,
                    desc: 'Easy on the eyes for night-time use',
                  },
                ].map((opt) => (
                  <button
                    key={opt.value}
                    id={`theme-${opt.value}`}
                    onClick={() =>
                      (isDark ? opt.value === 'light' : opt.value === 'dark') && toggleTheme()
                    }
                  ...
                  >
                    <opt.icon
                      size={24}
                      className={
                        isDark && opt.value === 'dark'
                          ? 'text-primary-500'
                          : !isDark && opt.value === 'light'
                            ? 'text-primary-500'
                            : 'text-gray-400'
                      }
                    />
          ...
          {activeSection === 'notifications' && (
            <div className="space-y-4">
              ...
              {[
                {
                  label: 'Yield Prediction Alerts',
                  sub: 'Get notified when AI predicts yield changes',
                  on: true,
                },
                {
                  label: 'Irrigation Reminders',
                  sub: 'Scheduled irrigation notifications',
                  on: true,
                },
                {
                  label: 'Climate Risk Alerts',
                  sub: 'Severe weather and drought warnings',
                  on: true,
                },
                { label: 'Soil Health Reports', sub: 'Weekly soil analysis summaries', on: false },
                {
                  label: 'Market Price Updates',
                  sub: 'Crop market price notifications',
                  on: false,
                },
              ].map((n, i) => (
                <div
                ...
                >
                  ...
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform shadow ${n.on ? 'translate-x-6' : 'translate-x-1'}`}
                    />
                  </button>
                </div>
              ))}
